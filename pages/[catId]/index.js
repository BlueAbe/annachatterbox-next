...
export default function Category({ posts }) {
  return (
    <>
      <Head>
        <title>{posts[0].attributes.category.data.attributes.name}</title>
        <meta name="description" content="Generated by create next app" />
        <link rel="icon" href="/favicon.ico" />
      </Head>
      <h2 className="heading-category">
        <span>{posts[0].attributes.category.data.attributes.name}</span>
      </h2>
      <main>
        {posts.map((p) => {
          return (
...
export async function getStaticProps(context) {
  ...
  const res1 = await fetch(
    ` https://annachatterbox.herokuapp.com/api/articles?filters[category][name][$eq]=${params.catId}&populate=*&sort[0]=commit:desc`
  );
